...
// LOAD FILE
term(`File `)
term.bold.underline(`batch-${batchId}.json`)
term(' process started\n')
let data = []
let result = []

let progressBar = term.progressBar({
    title: `Loading batch-${batchId}.json`,
    eta: true,
    percent: true,
    width: 80
})

const fileStat = fs.statSync(`./batches/batch-${batchId}.json`)
let readedSize = 0

fs.createReadStream(`./batches/batch-${batchId}.json`)
.on('data', v => {
    readedSize += v.length
    progressBar.update({
        progress: readedSize / fileStat.size
    })
    data.push(Buffer.from(v))
})
.on('end', () => {
    ...
    .on('end', async () => {
        ...
        for (const puuid of Object.keys(data)) {
            ...
            const outData = {
                ...
                tagLine: player.tagLine,
                line: playerList.find(v => v.name === player.playerName)?.line ?? '',
                gameCount: 0,
                victory: 0,
                defeat: 0,
                kills: 0,
